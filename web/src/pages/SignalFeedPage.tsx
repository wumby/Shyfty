...
function formatFreshnessLabel(lastUpdated: string | null): string {
...
function FreshnessBar() {
  ...
  const stale = ingestStatus.last_updated ? Date.now() - new Date(ingestStatus.last_updated).getTime() > 6 * 60 * 60 * 1000 : true;
  const dotColor =
        ...
        <span>
          {ingestStatus.status === 'running'
            ...
            : ingestStatus.status === 'error'
              ? 'Update failed'
              : stale
                ? `Stale: ${formatFreshnessLabel(ingestStatus.last_updated)}`
                : formatFreshnessLabel(ingestStatus.last_updated)}
        </span>
