    ...
    return (
      <div className="space-y-4">
        <div className="panel-surface px-6 py-8 text-center">
          <p className="text-sm text-muted">Sign in to manage your follows, saved views, and preferences.</p>
          <button
            type="button"
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {hasFollows && (
            <span className="text-[11px] text-muted">
              {followedPlayers.length + followedTeams.length} tracked
            </span>
          )}
        </div>
        ...
        {hasFollows ? (
          <div className="space-y-2">
            {followedPlayers.map((player) => (
              <div
                key={player.id}
                className="flex items-center justify-between gap-4 rounded-[18px] border border-border bg-white/[0.025] px-4 py-3"
              >
                <div className="min-w-0">
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  onClick={() => void toggleFollowPlayer(player.id, true)}
                  className="shrink-0 text-[11px] font-semibold text-muted transition hover:text-danger"
                >
                  Unfollow
                </button>
              </div>
            ...
            {followedTeams.map((team) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => void toggleFollowTeam(team.id, true)}
                  className="shrink-0 text-[11px] font-semibold text-muted transition hover:text-danger"
                >
                  Unfollow
                </button>
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-muted">
                    {[view.league, view.signal_type].filter(Boolean).join(' · ') || 'All signals'}
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-muted transition hover:text-danger"
                  >
                    Delete
                  </button>
                </div>
