...
def reset_legacy_seeded_nfl(db: Session, *, dry_run: bool = False) -> ResetResult:
    ...
    seeded_signal_ids = select(Shyft.id).where(Shyft.league_id.in_(nfl_league_ids))
    seeded_comment_ids = select(ShyftComment.id).where(ShyftComment.shyft_id.in_(seeded_signal_ids))
    ...
    counts = {
        ...
        "shyfts_deleted": _count_scalar(db, select(func.count()).select_from(Shyft).where(Shyft.id.in_(seeded_signal_ids))),
        "reactions_deleted": _count_scalar(db, select(func.count()).select_from(ShyftReactionRecord).where(ShyftReactionRecord.shyft_id.in_(seeded_signal_ids))),
        "comments_deleted": _count_scalar(db, select(func.count()).select_from(ShyftComment).where(ShyftComment.id.in_(seeded_comment_ids))),
    ...
    if not dry_run:
        ...
        db.execute(delete(ShyftReactionRecord).where(ShyftReactionRecord.shyft_id.in_(seeded_signal_ids)))
        db.execute(
        ...
        db.execute(delete(ShyftComment).where(ShyftComment.id.in_(seeded_comment_ids)))
        db.execute(delete(Shyft).where(Shyft.id.in_(seeded_signal_ids)))
...
def reset_sports_data(db: Session, *, dry_run: bool = False) -> ResetResult:
    ...
    signal_ids = select(Shyft.id)
    comment_ids = select(ShyftComment.id)
    ...
    counts = {
        ...
        "shyfts_deleted": _count_scalar(db, select(func.count()).select_from(Shyft)),
        "reactions_deleted": _count_scalar(db, select(func.count()).select_from(ShyftReactionRecord)),
        "comments_deleted": _count_scalar(db, select(func.count()).select_from(ShyftComment)),
    ...
    if not dry_run:
        ...
        db.execute(delete(ShyftReactionRecord).where(ShyftReactionRecord.shyft_id.in_(signal_ids)))
        db.execute(
            delete(UserFollow).where(
                ((UserFollow.entity_type == "player") & (UserFollow.entity_id.in_(player_ids)))
                | ((UserFollow.entity_type == "team") & (UserFollow.entity_id.in_(team_ids)))
        ...
        db.execute(delete(ShyftComment))
        db.execute(delete(Shyft))
